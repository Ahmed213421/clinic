<!-- Modal -->
<div class="modal fade" id="modaledit{{ $clinic->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
                        ...
                        <select class="custom-select" id="categoryselect" name="doctor_id[]" multiple>
                            ...
                            @foreach (App\Models\Admin::role('doctor')->get() as $doctor)
                                <option value="{{ $doctor->id }}"
                                    {{ $doctor->id == $clinic->doctors->contains($doctor->id) ? 'selected' : '' }}>
                                    {{ $doctor->name }}</option>
                            @endforeach
                        </select>
                    </div>
