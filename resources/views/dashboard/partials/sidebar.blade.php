    ...
    <nav class="vertnav navbar navbar-light">
        ...
        <ul class="navbar-nav flex-fill w-100 mb-2">
            <li class="nav-item w-100">
                <a class="nav-link {{ Route::is('admin.dashboard') ? 'activelink' : '' }}"
        ...
        <ul class="navbar-nav flex-fill w-100 mb-2">
            ...
            @if (auth('admin')->user()->can('view-user'))
                <li
                    class="nav-item dropdown {{ Route::is('admin.users.*') || Route::is('admin.roles.*') || Route::is('admin.permissions.*') ? 'active' : '' }}">
                    <a href="#permission" data-toggle="collapse" aria-expanded="false"
                        class="dropdown-toggle nav-link {{ Route::is('admin.news.*') ? 'activelink' : '' }}">
                        <i class="fa-regular fa-newspaper"></i>
                        <span class="ml-3 item-text"> Admins </span>
                    </a>
                    <ul class="collapse list-unstyled pl-4 w-100 {{ Route::is('admin.news.*') ? 'show' : '' }}"
                        id="permission">
                        <li class="nav-item">
                            <a class="nav-link pl-3" href="{{ route('admin.users.index') }}"><span
                                    class="ml-1 item-text">all Admins </span>
                                <a class="nav-link pl-3" href="{{ route('admin.permissions.index') }}"><span
                                        class="ml-1 item-text">Permissions </span>
                                </a>
                        </li>
                    </ul>
                </li>
        ...
        <ul class="navbar-nav flex-fill w-100 mb-2">
            @if (auth('admin')->user()->can('view-clinic'))
                <li class="nav-item w-100">
                    <a class="nav-link {{ Route::is('admin.clinic.index') ? 'activelink' : '' }}"
            ...
            @if (auth('admin')->user()->can('view-doctor'))
                <li class="nav-item w-100">
                    <a class="nav-link {{ Route::is('admin.doctor.index') ? 'activelink' : '' }}"
            ...
            @if (auth('web')->user() == 'web')
                <li class="nav-item w-100">
                    <a class="nav-link {{ Route::is('admin.appointment.index') ? 'activelink' : '' }}"
            ...
            @if (auth('admin')->user()->hasRole('super-admin', 'admin'))
                <li class="nav-item w-100">
                    <a class="nav-link {{ Route::is('admin.booked') ? 'activelink' : '' }}"
                        href="{{ route('admin.booked') }}">
                        <i class="fa-solid fa-gear"></i>
